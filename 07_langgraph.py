"""
Sample 7: Explicit LangGraph — Custom Graph with Conditional Routing
=====================================================================
Builds a LangGraph state machine from scratch (not hidden behind create_agent).
...
Run with New Relic:
  NEW_RELIC_CONFIG_FILE=newrelic.ini newrelic-admin run-program python 07_langgraph.py
"""
...
def safe_run(query: str):
    run_with_retry(lambda: run_query(query))


nr_app = newrelic.agent.register_application(timeout=30)

try:
    with newrelic.agent.BackgroundTask(nr_app, name="langgraph", group="LangChain"):
