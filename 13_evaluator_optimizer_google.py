"""
Sample 13G: Evaluator-Optimizer — Self-Correcting Feedback Loop (Google Gemini)
================================================================================
Same as 13_evaluator_optimizer.py but uses Google's Gemini model.
The graph cycle pattern is LangGraph-level, not LLM-specific.
...
Run with New Relic:
  NEW_RELIC_CONFIG_FILE=newrelic.ini newrelic-admin run-program python 13_evaluator_optimizer_google.py
"""
...
eval_llm = get_google_llm(temperature=0)  # Evaluator should be deterministic
...
try:
    with newrelic.agent.BackgroundTask(nr_app, name="evaluator-optimizer-google", group="LangChain"):
