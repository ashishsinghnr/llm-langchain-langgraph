"""
Sample 11: Human-in-the-Loop — LangGraph interrupt()
=====================================================
Builds on LangGraph (07) to add human approval gates. The graph pauses
before sensitive actions (refund, cancel) and waits for human input
before proceeding. Non-sensitive actions (status lookup) flow through
automatically.
...
Why this matters:
  Production agents must NOT take irreversible actions (refunds, deletions,
  sends) without a human check. interrupt() is LangGraph's built-in way to
  pause, surface context, and resume — no polling or external queues needed.

Run with New Relic:
  NEW_RELIC_CONFIG_FILE=newrelic.ini newrelic-admin run-program python 11_human_in_the_loop.py
"""
...
try:
    with newrelic.agent.BackgroundTask(nr_app, name="human-in-the-loop", group="LangChain"):
